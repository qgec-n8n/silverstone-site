...
| QB-003 | Calendly                | Native `/web` booking console, Netlify Functions, integration safety docs | Native availability and invitee creation are implemented behind server-only credentials; deterministic staging fixtures, range chunking, validation, duplicate control, and fallback states are verified, while provider account/plan/event-question settings remain unverified | Authorized production configuration review and provider-side smoke proof without exposing credentials; no real appointment during validation |
