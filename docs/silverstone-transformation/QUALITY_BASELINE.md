...
| QB-010 | Sitemap drift           | A-02 F-019                                                   | Correct URL membership but stale lastmod/build drift in legacy SEO artifacts                                                           | Generated sitemap equals route source and terminal/indexable checks pass          |
...
| QB-014 | Performance budgets     | active ExecPlan, Replit handoff                              | Later `/web` bundle report recorded `282.53 KB gzip` target miss; quantitative LCP/FPS/long-task not fully measured for mounted routes | Bundle budget, Lighthouse/tracing, FPS/long-task checks on representative routes  |
| QB-015 | Format baseline         | active ExecPlan                                              | Full `/web` `npm run format:check` has documented pre-existing failures                                                                | Either resolve in dedicated formatting task or use changed-file formatting checks |
