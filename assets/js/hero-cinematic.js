 ...
 * - Aurora cascade reveal when descending, cinematic bars focus when ascending
...
(function(){
  ...
  function init(){
    ...
    if(!next.querySelector(".fx-body-reveal")){var r=document.createElement("div");r.className="fx-body-reveal";r.setAttribute("aria-hidden","true");next.insertBefore(r,next.firstChild);}
    ...
    function animate(yTarget,dir){anim=true;lock(true);var y0=window.scrollY,delta=yTarget-y0,t0=performance.now(),dur=CONFIG.DURATION_MS;
      ...
      function tick(now){if(isOverlayOpen()){anim=false;lock(false);return;}var t=clamp((now-t0)/dur,0,1),e=ease(t),y=y0+delta*e;window.scrollTo(0,y);
        ...
        var bloom=dir==="down"?0:Math.pow(Math.sin(Math.PI*t),1.55);setVar("--cineBloom",bloom);
        var beamBoost=dir==="down"?0:Math.pow(Math.sin(Math.PI*t),2.2);setVar("--cineBeamBoost",beamBoost);
        var aurora=Math.pow(Math.sin(Math.PI*t),dir==="down"?1.35:0.9)*(dir==="down"?1:0.45);setVar("--cineAurora",aurora);
        var reveal=dir==="down"?Math.pow(e,0.85):Math.pow(1-e,1.1)*0.65;setVar("--cineReveal",reveal);
        var spark=dir==="down"?Math.pow(Math.sin(Math.PI*t),3.1):Math.pow(Math.sin(Math.PI*t),1.6)*0.4;setVar("--cineSpark",spark);
        ...
        if(t<1&&anim)requestAnimationFrame(tick);else{window.scrollTo(0,yTarget);hero.style.setProperty("--heroProgress",dir==="down"?1:0);
          ...
          setVar("--cineAurora",0);setVar("--cineReveal",dir==="down"?0.2:0);setVar("--cineSpark",0);
    ...
    (function initState(){var g=geo(hero,next),past=window.scrollY>=g.bodyY;hero.style.setProperty("--heroProgress",past?1:0);
      ...
      setVar("--cineAurora",0);setVar("--cineReveal",past?0.2:0);setVar("--cineSpark",0);
