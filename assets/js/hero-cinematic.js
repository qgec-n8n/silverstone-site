...
(function(){
  ...
  function init(){
    ...
    if(!hero.querySelector(".fx-accents")){var a=document.createElement("div");a.className="fx-accents";a.setAttribute("aria-hidden","true");hero.appendChild(a);}
    ...
    function animate(yTarget,dir){anim=true;lock(true);var y0=window.scrollY,delta=yTarget-y0,t0=performance.now(),dur=CONFIG.DURATION_MS;
      ...
      function tick(now){if(isOverlayOpen()){anim=false;lock(false);return;}var t=clamp((now-t0)/dur,0,1),e=ease(t),y=y0+delta*e;window.scrollTo(0,y);
        ...
        if(dir==="down"){var pulse=Math.pow(e,0.65);var iris=Math.pow(e,0.82);var scan=Math.pow(Math.sin(Math.min(1,e)*Math.PI*0.88),1.4);setVar("--cinePulse",pulse);setVar("--cineIris",iris);setVar("--cineScan",scan);}else{setVar("--cinePulse",0);setVar("--cineIris",0);setVar("--cineScan",0);}
        if(t<1&&anim)requestAnimationFrame(tick);else{window.scrollTo(0,yTarget);hero.style.setProperty("--heroProgress",dir==="down"?1:0);
          ...
          setVar("--cineVeil",dir==="down"?0:0.45);setVar("--cinePulse",0);setVar("--cineIris",0);setVar("--cineScan",0);anim=false;lock(false);}}
    ...
    (function initState(){var g=geo(hero,next),past=window.scrollY>=g.bodyY;hero.style.setProperty("--heroProgress",past?1:0);
      ...
      setVar("--cineVeil",past?0:0.45);setVar("--cinePulse",0);setVar("--cineIris",0);setVar("--cineScan",0);})();window.addEventListener("wheel",onWheel,{passive:false});
