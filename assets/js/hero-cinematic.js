/*! assets/js/hero-cinematic.js — v6 (Body-to-hero cinematic only)
 * - Retains the cinematic return-to-hero effect when scrolling upward.
 * - Removes all hero-to-body trigger logic to avoid conflicts with orb routing.
 */
(function () {
  "use strict";
  ...
  function BezierEasing(x1, y1, x2, y2) {
    ...
    var SIZE = 11, STEP = 1 / (SIZE - 1), arr = new Float32Array(SIZE);
    for (var i = 0; i < SIZE; ++i) arr[i] = calc(i * STEP, x1, x2);
    function getT(x) {
      var start = 0, i = 1;
      while (i < SIZE - 1 && arr[i] <= x) { start += STEP; ++i; }
      --i;
      var dist = (x - arr[i]) / (arr[i + 1] - arr[i]), guess = start + dist * STEP, s = slope(guess, x1, x2);
      if (s >= .001) {
        for (var j = 0; j < 4; ++j) { var c = calc(guess, x1, x2) - x; guess -= c / slope(guess, x1, x2); }
        return guess;
      }
      return guess;
    }
  ...
  function geo(hero, next) {
    ...
    return { off: off, visH: visH, bodyY: Math.max(0, next.getBoundingClientRect().top + window.scrollY - off) };
  }
  function lock(l) {
    ...
    if (l && !r.classList.contains("cine-locked")) { r.classList.add("cine-locked"); document.body.classList.add("cine-locked"); window.addEventListener("wheel", w, { passive: false }); window.addEventListener("touchmove", t, { passive: false }); window.addEventListener("keydown", k, { passive: false }); }
    else if (!l && r.classList.contains("cine-locked")) { window.removeEventListener("wheel", w); window.removeEventListener("touchmove", t); window.removeEventListener("keydown", k); r.classList.remove("cine-locked"); document.body.classList.remove("cine-locked"); }
  }
  ...
  function init() {
    ...
    function sizeHero() { hero.style.minHeight = geo(hero, next).visH + "px"; }
    sizeHero();
    var anim = false, lastY = window.scrollY;
    var DEPTH_DOWN_MAX = 1.15;
    function animateToHero() {
      anim = true; lock(true); var y0 = window.scrollY, delta = 0 - y0, t0 = performance.now(), dur = CONFIG.DURATION_MS;
      var depthFrom = DEPTH_DOWN_MAX, depthTo = 0;
      function tick(now) {
        if (isOverlayOpen()) { anim = false; lock(false); return; }
        var t = clamp((now - t0) / dur, 0, 1), e = ease(t), y = y0 + delta * e; window.scrollTo(0, y);
        var progress = 1 - e; hero.style.setProperty("--heroProgress", progress);
        var depth = depthFrom + (depthTo - depthFrom) * e; hero.style.setProperty("--heroDepth", depth);
        var bars = Math.sin(Math.PI * t) ** 0.9; setVar("--cineBars", bars);
        setVar("--cineBloom", Math.pow(Math.sin(Math.PI * t), 1.35));
        setVar("--cineBeamBoost", Math.pow(Math.sin(Math.PI * t), 2.0));
        setVar("--cineAurora", 0); setVar("--cinePulse", 0);
        setVar("--cineVeil", 0.45 * e);
        if (t < 1 && anim) requestAnimationFrame(tick); else {
          window.scrollTo(0, 0);
          hero.style.setProperty("--heroProgress", 0);
          hero.style.setProperty("--heroDepth", depthTo);
          setVar("--cineBars", 0); setVar("--cineBloom", 0); setVar("--cineBeamBoost", 0);
          setVar("--cineAurora", 0); setVar("--cinePulse", 0); setVar("--cineVeil", 0.45);
          anim = false; lock(false);
        }
      }
      requestAnimationFrame(tick);
    }
    function tryUp() { if (anim || isOverlayOpen()) return; animateToHero(); }
    function syncHeroState(gInfo) {
      if (anim) return; var g = gInfo || geo(hero, next), sy = window.scrollY;
      var past = sy >= g.bodyY - 1; var progress = past ? 1 : clamp(sy / Math.max(g.bodyY, 1), 0, 1);
      hero.style.setProperty("--heroProgress", past ? 1 : progress);
      hero.style.setProperty("--heroDepth", DEPTH_DOWN_MAX * (past ? 1 : progress));
      setVar("--cineVeil", past ? 0 : 0.45 * (1 - progress));
    }
    function onWheel(e) {
    ...
    (function initState() {
      ...
      if (window.location.hash) { past = true; }
    ...
    window.addEventListener("scroll", onScroll, { passive: true }); window.addEventListener("resize", function () { sizeHero(); syncHeroState(); }, { passive: true });
  }
