  ...
  function init(){
    ...
    function animate(yTarget,dir){anim=true;lock(true);var y0=window.scrollY,delta=yTarget-y0,t0=performance.now(),dur=CONFIG.DURATION_MS;
      ...
      function tick(now){if(isOverlayOpen()){anim=false;lock(false);return;}var t=clamp((now-t0)/dur,0,1),e=ease(t),y=y0+delta*e;window.scrollTo(0,y);
        ...
        var aurora=0,reveal=0,pulse=0;if(dir==="down"){aurora=Math.pow(e,0.82);var wave=Math.sin(Math.PI*Math.min(1,t*0.92));pulse=wave>0?Math.pow(wave,1.8):0;reveal=Math.pow(e,1.1);}else{pulse=0;}
        setVar("--cineAurora",aurora);setVar("--cineReveal",reveal);setVar("--cinePulse",pulse);
        ...
        if(t<1&&anim)requestAnimationFrame(tick);else{window.scrollTo(0,yTarget);hero.style.setProperty("--heroProgress",dir==="down"?1:0);
          hero.style.setProperty("--heroDepth",depthTo);setVar("--cineBars",0);setVar("--cineAurora",0);setVar("--cineReveal",0);setVar("--cinePulse",0);setVar("--cineBloom",0);setVar("--cineBeamBoost",0);
    ...
    (function initState(){var g=geo(hero,next),past=window.scrollY>=g.bodyY;hero.style.setProperty("--heroProgress",past?1:0);
      hero.style.setProperty("--heroDepth",past?DEPTH_DOWN_MAX:0);setVar("--cineBars",0);setVar("--cineAurora",0);setVar("--cineReveal",0);setVar("--cinePulse",0);setVar("--cineBloom",0);setVar("--cineBeamBoost",0);
