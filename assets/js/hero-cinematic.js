...
(function(){
  ...
  function init(){
    ...
    if(!hero.querySelector(".fx-prism")){var p=document.createElement("div");p.className="fx-prism";p.setAttribute("aria-hidden","true");var c=hero.querySelector(".content");if(c)hero.insertBefore(p,c);else hero.appendChild(p);}
    ...
    if(!next.querySelector(".fx-holo")){var h=document.createElement("div");h.className="fx-holo";h.setAttribute("aria-hidden","true");var veil=next.querySelector(".fx-veil");if(veil&&veil.nextSibling)next.insertBefore(h,veil.nextSibling);else next.appendChild(h);}
    ...
    function animate(yTarget,dir){anim=true;lock(true);var y0=window.scrollY,delta=yTarget-y0,t0=performance.now(),dur=CONFIG.DURATION_MS;
      ...
      function tick(now){if(isOverlayOpen()){anim=false;lock(false);return;}var t=clamp((now-t0)/dur,0,1),e=ease(t),y=y0+delta*e;window.scrollTo(0,y);
        ...
        var prism=dir==="down"?Math.pow(Math.sin(Math.PI*Math.min(1,t*1.05)),1.12):0;setVar("--cinePrism",prism);
        var prismShift=dir==="down"?e:0;setVar("--cinePrismShift",prismShift);
        var holo=dir==="down"?Math.pow(Math.sin(Math.PI*t),1.4):0;setVar("--cineHolo",holo);
        ...
        if(t<1&&anim)requestAnimationFrame(tick);else{window.scrollTo(0,yTarget);hero.style.setProperty("--heroProgress",dir==="down"?1:0);
          ...
          setVar("--cinePrism",0);setVar("--cinePrismShift",0);setVar("--cineHolo",0);
