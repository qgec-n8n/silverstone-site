...
(function(){
  ...
  function ensureTransitionFX(){var w=document.querySelector(".fx-transition");if(!w){w=document.createElement("div");w.className="fx-transition";
      w.innerHTML='<div class="aurora"></div><div class="spark"></div>';document.body.appendChild(w);}}
  function init(){
    ...
    ensureTransitionFX();if(!next.querySelector(".fx-veil")){if(getComputedStyle(next).position==="static")next.style.position="relative";var v=document.createElement("div");v.className="fx-veil";v.setAttribute("aria-hidden","true");next.insertBefore(v,next.firstChild);}
    ...
    function animate(yTarget,dir){anim=true;lock(true);var y0=window.scrollY,delta=yTarget-y0,t0=performance.now(),dur=CONFIG.DURATION_MS;
      ...
      function tick(now){if(isOverlayOpen()){anim=false;lock(false);return;}var t=clamp((now-t0)/dur,0,1),e=ease(t),y=y0+delta*e;window.scrollTo(0,y);
        ...
        var signal=Math.sin(Math.PI*t)**0.85;setVar("--cineSignal",signal);
        var pulse=Math.sin(Math.PI*t)**(dir==="down"?1.35:1.1);setVar("--cinePulse",pulse);
        setVar("--cineSpark",dir==="down"?e:1-e);
        ...
        if(t<1&&anim)requestAnimationFrame(tick);else{window.scrollTo(0,yTarget);hero.style.setProperty("--heroProgress",dir==="down"?1:0);
          hero.style.setProperty("--heroDepth",depthTo);setVar("--cineSignal",0);setVar("--cinePulse",0);setVar("--cineSpark",0);setVar("--cineBloom",0);setVar("--cineBeamBoost",0);
    ...
    (function initState(){var g=geo(hero,next),past=window.scrollY>=g.bodyY;hero.style.setProperty("--heroProgress",past?1:0);
      hero.style.setProperty("--heroDepth",past?DEPTH_DOWN_MAX:0);setVar("--cineSignal",0);setVar("--cinePulse",0);setVar("--cineSpark",0);setVar("--cineBloom",0);setVar("--cineBeamBoost",0);
