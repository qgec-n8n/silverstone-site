...
(function(){
  ...
  function ensureHeroIris(hero){if(!hero.querySelector(".fx-iris")){var i=document.createElement("div");i.className="fx-iris";
      i.setAttribute("aria-hidden","true");hero.appendChild(i);}}
  function ensureBodyPrism(section){if(!section.querySelector(".fx-prism")){var p=document.createElement("div");p.className="fx-prism";
      p.setAttribute("aria-hidden","true");section.insertBefore(p,section.firstChild);}}
  function init(){
    ...
    ensureBars();ensureHeroIris(hero);if(!next.querySelector(".fx-veil")){if(getComputedStyle(next).position==="static")next.style.position="relative";var v=document.createElement("div");v.className="fx-veil";v.setAttribute("aria-hidden","true");next.insertBefore(v,next.firstChild);}ensureBodyPrism(next);
    ...
    function animate(yTarget,dir){anim=true;lock(true);var y0=window.scrollY,delta=yTarget-y0,t0=performance.now(),dur=CONFIG.DURATION_MS;
      ...
      function tick(now){if(isOverlayOpen()){anim=false;lock(false);return;}var t=clamp((now-t0)/dur,0,1),e=ease(t),y=y0+delta*e;window.scrollTo(0,y);
        ...
        var veil=(dir==="down"?0.42*(1-e):0.45*e);setVar("--cineVeil",veil);
        var iris=dir==="down"?Math.pow(Math.sin(Math.PI*Math.min(1,e*0.96))||0,0.9):0;setVar("--cineIris",iris);
        var prism=dir==="down"?Math.pow(Math.sin(Math.PI*Math.min(1,e*1.05))||0,1.35):0;setVar("--cinePrism",prism);
        var pulse=dir==="down"?Math.pow(Math.sin(Math.PI*Math.min(1,e*1.25))||0,2.1):0;setVar("--cinePulse",pulse);
        if(t<1&&anim)requestAnimationFrame(tick);else{window.scrollTo(0,yTarget);hero.style.setProperty("--heroProgress",dir==="down"?1:0);
          ...
          setVar("--cineVeil",dir==="down"?0:0.45);setVar("--cineIris",0);setVar("--cinePrism",0);setVar("--cinePulse",0);
    ...
    (function initState(){var g=geo(hero,next),past=window.scrollY>=g.bodyY;hero.style.setProperty("--heroProgress",past?1:0);
      ...
      setVar("--cineVeil",past?0:0.45);setVar("--cineIris",0);setVar("--cinePrism",0);setVar("--cinePulse",0);})();window.addEventListener("wheel",onWheel,{passive:false});
