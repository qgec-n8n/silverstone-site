...
(function(){
  ...
  function init(){
    ...
    if(!hero.querySelector(".fx-prism")){var p=document.createElement("div");p.className="fx-prism";p.setAttribute("aria-hidden","true");hero.appendChild(p);}
    if(!hero.querySelector(".fx-pulse")){var u=document.createElement("div");u.className="fx-pulse";u.setAttribute("aria-hidden","true");hero.appendChild(u);}
    ...
    function animate(yTarget,dir){anim=true;lock(true);var y0=window.scrollY,delta=yTarget-y0,t0=performance.now(),dur=CONFIG.DURATION_MS;
      ...
      function tick(now){if(isOverlayOpen()){anim=false;lock(false);return;}var t=clamp((now-t0)/dur,0,1),e=ease(t),y=y0+delta*e;window.scrollTo(0,y);
        ...
        var bloom=dir==="down"?Math.pow(Math.sin(Math.PI*t),1.12):Math.pow(Math.sin(Math.PI*t),1.35);setVar("--cineBloom",bloom);
        var beamBoost=dir==="down"?0:Math.pow(Math.sin(Math.PI*t),2.0);setVar("--cineBeamBoost",beamBoost);
        var prism=dir==="down"?Math.pow(Math.sin(Math.PI*t),0.9):0;setVar("--cinePrism",prism);
        var pulse=dir==="down"?Math.pow(e,0.8):0;setVar("--cinePulse",pulse);
        var scan=dir==="down"?Math.pow(e,1.35):Math.pow(1-e,1.25)*0.55;setVar("--cineScan",scan,0,1);
        ...
        if(t<1&&anim)requestAnimationFrame(tick);else{window.scrollTo(0,yTarget);hero.style.setProperty("--heroProgress",dir==="down"?1:0);
          ...
          setVar("--cinePrism",0);setVar("--cinePulse",0);setVar("--cineScan",0);
