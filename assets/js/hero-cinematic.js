...
(function(){
  ...
  function init(){
    ...
    if(!next.querySelector(".fx-horizon")){if(getComputedStyle(next).position==="static")next.style.position="relative";var h=document.createElement("div");h.className="fx-horizon";h.setAttribute("aria-hidden","true");next.insertBefore(h,next.firstChild);}
    ...
    function animate(yTarget,dir){anim=true;lock(true);var y0=window.scrollY,delta=yTarget-y0,t0=performance.now(),dur=CONFIG.DURATION_MS;
      ...
      function tick(now){if(isOverlayOpen()){anim=false;lock(false);return;}var t=clamp((now-t0)/dur,0,1),e=ease(t),y=y0+delta*e;window.scrollTo(0,y);
        ...
        var sin=Math.sin(Math.PI*t);
        var bars=dir==="down"?0:sin**0.9;setVar("--cineBars",bars);
        var bloom=dir==="down"?sin**1.2*0.55:sin**1.35;setVar("--cineBloom",bloom);
        var beamBoost=dir==="down"?0:sin**1.6;setVar("--cineBeamBoost",beamBoost);
        var aurora=dir==="down"?sin**1.25:0;setVar("--cineAurora",aurora);
        var reveal=dir==="down"?Math.min(1,sin**1.05*1.1):0;setVar("--cineReveal",reveal);
        var focus=dir==="down"?0:sin**1.45;setVar("--cineReturnPulse",focus);
        ...
        if(t<1&&anim)requestAnimationFrame(tick);else{window.scrollTo(0,yTarget);hero.style.setProperty("--heroProgress",dir==="down"?1:0);
          ...
          setVar("--cineAurora",0);setVar("--cineReveal",0);setVar("--cineReturnPulse",0);
