...
(function(){
  ...
  function init(){
    ...
    if(!next.querySelector(".fx-signal")){var s=document.createElement("div");s.className="fx-signal";s.setAttribute("aria-hidden","true");next.insertBefore(s,next.firstChild);}
    ...
    function animate(yTarget,dir){anim=true;lock(true);var y0=window.scrollY,delta=yTarget-y0,t0=performance.now(),dur=CONFIG.DURATION_MS;
      ...
      function tick(now){if(isOverlayOpen()){anim=false;lock(false);return;}var t=clamp((now-t0)/dur,0,1),e=ease(t),y=y0+delta*e;window.scrollTo(0,y);
        ...
        var aurora=Math.pow(Math.sin(Math.PI*t),dir==="down"?1.05:1.2);setVar("--cineAurora",dir==="down"?aurora:aurora*0.6);
        var signal=dir==="down"?Math.pow(Math.sin(Math.PI*t),1.65):Math.pow(1-e,2.2);setVar("--cineSignal",signal);
        if(t<1&&anim)requestAnimationFrame(tick);else{window.scrollTo(0,yTarget);hero.style.setProperty("--heroProgress",dir==="down"?1:0);
          ...
          setVar("--cineVeil",dir==="down"?0:0.45);setVar("--cineAurora",0);setVar("--cineSignal",0);
    ...
    (function initState(){var g=geo(hero,next),past=window.scrollY>=g.bodyY;hero.style.setProperty("--heroProgress",past?1:0);
      ...
      setVar("--cineVeil",past?0:0.45);setVar("--cineAurora",0);setVar("--cineSignal",0);})();window.addEventListener("wheel",onWheel,{passive:false});
