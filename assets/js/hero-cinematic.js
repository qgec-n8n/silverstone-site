...
(function(){
  ...
  function init(){
    ...
    if(!next.querySelector(".fx-signal")){if(getComputedStyle(next).position==="static")next.style.position="relative";var s=document.createElement("div");s.className="fx-signal";s.setAttribute("aria-hidden","true");next.insertBefore(s,next.firstChild);}
    ...
    function animate(yTarget,dir){anim=true;lock(true);var y0=window.scrollY,delta=yTarget-y0,t0=performance.now(),dur=CONFIG.DURATION_MS;
      ...
      function tick(now){if(isOverlayOpen()){anim=false;lock(false);return;}var t=clamp((now-t0)/dur,0,1),e=ease(t),y=y0+delta*e;window.scrollTo(0,y);
        ...
        if(dir==="down"){var signalDown=Math.pow(Math.sin(Math.PI*Math.min(1,t*1.05)),1.6);setVar("--cineSignalDown",signalDown);setVar("--cineSignalUp",0);}else{var signalUp=Math.pow(Math.sin(Math.PI*Math.min(1,t*1.05)),1.4);setVar("--cineSignalUp",signalUp);setVar("--cineSignalDown",0);}
        if(t<1&&anim)requestAnimationFrame(tick);else{window.scrollTo(0,yTarget);hero.style.setProperty("--heroProgress",dir==="down"?1:0);
          ...
          setVar("--cineVeil",dir==="down"?0:0.45);setVar("--cineSignalDown",0);setVar("--cineSignalUp",0);anim=false;lock(false);}}
    ...
    (function initState(){var g=geo(hero,next),past=window.scrollY>=g.bodyY;hero.style.setProperty("--heroProgress",past?1:0);
      ...
      setVar("--cineVeil",past?0:0.45);setVar("--cineSignalDown",0);setVar("--cineSignalUp",0);})();window.addEventListener("wheel",onWheel,{passive:false});
