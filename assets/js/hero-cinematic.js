  ...
  function init(){
    ...
    function animate(yTarget,dir){anim=true;lock(true);var y0=window.scrollY,delta=yTarget-y0,t0=performance.now(),dur=CONFIG.DURATION_MS;
      ...
      function tick(now){if(isOverlayOpen()){anim=false;lock(false);return;}var t=clamp((now-t0)/dur,0,1),e=ease(t),y=y0+delta*e;window.scrollTo(0,y);
        ...
        var bars=dir==="up"?Math.pow(pulse,0.9):0;setVar("--cineBars",bars);
        var bloom=dir==="down"?0:Math.pow(pulse,1.35);setVar("--cineBloom",bloom);
        var beamBoost=dir==="up"?Math.pow(pulse,2.0):0;setVar("--cineBeamBoost",beamBoost);
        var aurora=dir==="down"?Math.pow(pulse,1.18):0;setVar("--cineAurora",aurora);
        var reveal=dir==="down"?Math.pow(Math.sin(Math.PI*Math.min(1,t*1.12)),1.6):0;setVar("--cineReveal",reveal);
        var focus=dir==="up"?Math.pow(pulse,1.45):0;setVar("--cineFocus",focus);
        ...
        if(t<1&&anim)requestAnimationFrame(tick);else{window.scrollTo(0,yTarget);hero.style.setProperty("--heroProgress",dir==="down"?1:0);
          ...
          setVar("--cineAurora",0);setVar("--cineReveal",0);setVar("--cineFocus",0);
