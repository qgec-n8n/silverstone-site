/**
 * DOUBLE MARQUEE (Services Innovation Gallery)
 * - Injects a two-track marquee directly beneath the Innovation Gallery grid.
 * - Provides a shared lightbox for clicking any marquee tile.
 * - Keeps anchor navigation aligned with the sticky header when arriving via the orb.
 */
(function () {
  'use strict';

  const ASSET_PATH = 'assets/images/socialmedia/';
  const INNOVATION_ANCHOR_ID = 'innovation-gallery';
  ...
  function injectMarquee(grid) {
    const container = createContainer();
    const parent = grid.parentNode;

    container.classList.add('double-marquee-container');

    if (parent) {
      parent.insertBefore(container, grid.nextElementSibling);
      const section = grid.closest('.section');
      if (section) {
        section.classList.add('innovation-marquee-stack');
      }
    } else {
  ...
  function createContainer() {
    ...
    const rowTop = createMarqueeRow(MARQUEE_IMAGES, 'scroll-right fast');
    const rowBottom = createMarqueeRow(MARQUEE_IMAGES.slice().reverse(), 'scroll-left slow');
  ...
  function createMarqueeRow(images, animationClasses) {
    ...
    const firstPass = createImages(images);
    const secondPass = createImages(images);
  ...
  function ensureLightbox() {
    if (document.getElementById('marquee-lightbox')) return;

    const lightbox = document.createElement('div');
    lightbox.id = 'marquee-lightbox';
    lightbox.className = 'marquee-lightbox';

    const content = document.createElement('div');
    content.className = 'lightbox-content';

    const img = document.createElement('img');
    img.id = 'marquee-lightbox-img';
  ...
  function openLightbox(src) {
    ...
    const img = document.getElementById('marquee-lightbox-img');
  ...
  function alignInnovationAnchor() {
    if (window.location.hash !== `#${INNOVATION_ANCHOR_ID}`) return;

    const target = document.getElementById(INNOVATION_ANCHOR_ID);
    if (!target) return;
    ...
    const scrollToAnchor = () => {
      ...
      const offset = headerHeight + indicatorHeight + 6;
      const top = target.getBoundingClientRect().top + window.scrollY - offset;
      window.scrollTo({ top, behavior: 'auto' });
    };

    requestAnimationFrame(() => {
      // Small delay to allow fonts/layout to settle before adjusting scroll
