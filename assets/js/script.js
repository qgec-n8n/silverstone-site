...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Legacy per-section mobile parallax has been replaced by a global fixed
  // background layer implemented later in this file.  Bail out to prevent
  // duplicate logic from running.
  return;
...
/*
 * Mobile fixed parallax effect (global implementation)
 *
 * On mobile devices we replace the legacy per-section parallax with a single
 * fixed background element (#mobile-parallax-bg).  This element sits below
 * all interactive content and scrolls with the viewport, giving the
 * impression of a fixed image while text and other elements move over it.
 * A dark gradient overlay is included for legibility.  When a themed
 * section enters the viewport, we update the image and vertical offset on
 * the fixed element; when no section is visible, the element fades out.
 */
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!isMobile2 || prefersReducedMotion2) return;

  // Create or reuse the global mobile parallax background element.  This
  // element is defined in CSS (mobile.css) and hidden by default.
  ...
  if (!parallaxBg) {
    ...
    // insert at the top of <body> so it sits behind all content
    document.body.insertBefore(parallaxBg, document.body.firstChild);
  }

  // Map section identifiers to their base image filenames.  These match
  // the assets defined in assets/css/mobile.css for the themed sections.
  const bgMap = {
    'bg-lines': 'section-abstract-lines',
    'bg-circuit': 'section-circuit',
    'bg-mesh': 'section-mesh',
    'bg-waves': 'section-waves',
    'discovery-call-section': 'book-hero-calendly-mobile-2025'
  };

  // Select all sections that should trigger the parallax image.  Order
  // matters: the first section in the array will take precedence when
  // multiple sections intersect the viewport simultaneously.
  ...
  /**
   * Build a background-image string with a dark gradient overlay and an
   * image-set for DPR scaling.  The returned string is assigned to
   * style.backgroundImage on the global parallax element.
   * @param {string} baseName The base filename without resolution suffix.
   */
  function buildBackground(baseName) {
    const basePath = `../images/internet/mobile/${baseName}`;
  ...
  // Keep track of the currently active section to avoid redundant updates.
  let activeSection = null;

  function updateFixedParallax() {
    const depth = 0.3;
    ...
    for (const section of sections) {
      ...
      if (inView) {
        foundActive = true;
        // Determine the key for this section based on its class list
        let key = null;
        if (section.classList.contains('bg-lines')) {
          key = 'bg-lines';
        } else if (section.classList.contains('bg-circuit')) {
          key = 'bg-circuit';
        } else if (section.classList.contains('bg-mesh')) {
          key = 'bg-mesh';
        } else if (section.classList.contains('bg-waves')) {
          key = 'bg-waves';
        } else if (section.classList.contains('discovery-call-section')) {
          key = 'discovery-call-section';
        }

        // If the visible section has changed, update the image
        if (key && activeSection !== section) {
          const baseName = bgMap[key];
          if (baseName) {
            parallaxBg.style.backgroundImage = buildBackground(baseName);
            parallaxBg.style.opacity = '1';
            activeSection = section;
          }
        }

        // Update the vertical offset for parallax effect
        const yPos = rect.top * depth;
        parallaxBg.style.setProperty('--parY', `${yPos.toFixed(2)}px`);

        // Only process the first in-view section
        break;
      }
    }

    // If no section is visible, hide the background
    if (!foundActive) {
