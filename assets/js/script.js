...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Orchestrate the premium cinematic transition between the hero and
  ...
  function setupCinematicScene() {
    ...
    let bodySection = hero.nextElementSibling;
    while (bodySection && bodySection.tagName !== 'SECTION') {
      bodySection = bodySection.nextElementSibling;
    }
    if (!bodySection) return;

    const reduceMotionQuery = window.matchMedia('(prefers-reduced-motion: reduce)');
    let reduceMotionActive = reduceMotionQuery.matches;
    const root = document.documentElement;

    const metrics = {
      headerHeight: 0,
      bodyStart: 0,
      heroLift: parseFloat(getComputedStyle(root).getPropertyValue('--cinematic-hero-lift')) || 180,
      bodyShift: parseFloat(getComputedStyle(root).getPropertyValue('--cinematic-body-shift')) || 160,
    };

    const config = {
      duration: 1400,
      overlayMax: parseFloat(getComputedStyle(root).getPropertyValue('--cinematic-overlay-max')) || 0.65,
      heroScale: parseFloat(getComputedStyle(root).getPropertyValue('--cinematic-hero-scale')) || 1.05,
      heroLiftRatio: 0.28,
      bodyShiftRatio: 0.22,
      minHeroLift: 140,
      minBodyShift: 120,
    ...
    if (reduceMotionActive) {
      ...
      window.addEventListener('orientationchange', refreshMetrics, { passive: true });
      return;
    }
    ...
    hero.classList.add('cinematic-section', 'cinematic-hero');
    bodySection.classList.add('cinematic-section', 'cinematic-body');
    document.body.classList.remove('scene-body');
    document.body.classList.add('scene-hero', 'cinematic-ready');
    applyHeroState();
    window.scrollTo(0, 0);
    ...
    const wheelHandler = (event) => {
      ...
      if (sceneState === 'hero' && event.deltaY > 0) {
        event.preventDefault();
        startToBody();
        return;
      }
      if (sceneState === 'body') {
        if (window.scrollY <= metrics.bodyStart + config.scrollTolerance && event.deltaY < 0) {
          event.preventDefault();
          startToHero();
        } else if (window.scrollY < metrics.bodyStart) {
          window.scrollTo(0, metrics.bodyStart);
        }
      }
    };
    ...
    const keyHandler = (event) => {
      ...
      const preventKeys = ['ArrowDown', 'ArrowUp', 'PageDown', 'PageUp', 'Home'];
      if (animating && (preventKeys.includes(key) || isSpace)) {
        event.preventDefault();
        return;
      }
      if (sceneState === 'hero' && (key === 'PageDown' || key === 'ArrowDown' || (isSpace && !event.shiftKey))) {
        event.preventDefault();
        startToBody();
      } else if (
        sceneState === 'body' &&
        window.scrollY <= metrics.bodyStart + config.scrollTolerance &&
        (key === 'PageUp' || key === 'ArrowUp' || key === 'Home' || (isSpace && event.shiftKey))
      ) {
        event.preventDefault();
        startToHero();
      }
    ...
    const touchMoveHandler = (event) => {
      if (reduceMotionActive || touchStartY === null) return;
      ...
      if (sceneState === 'hero' && delta > config.touchThreshold) {
        event.preventDefault();
        touchStartY = null;
        startToBody();
      } else if (
        sceneState === 'body' &&
        delta < -config.touchThreshold &&
        window.scrollY <= metrics.bodyStart + config.scrollTolerance
      ) {
        event.preventDefault();
        touchStartY = null;
    ...
    const resizeHandler = () => {
      ...
      refreshMetrics();
      if (!overlay || reduceMotionActive) return;
      if (animating) return;
      ...
      } else if (previousState === 'body') {
        applyBodyState();
        if (currentScroll <= metrics.bodyStart + config.scrollTolerance) {
          window.scrollTo(0, metrics.bodyStart);
        }
      }
    };
    ...
    reduceMotionQuery.addEventListener('change', (event) => {
      reduceMotionActive = event.matches;
      if (reduceMotionActive) {
        teardownCinematic();
        refreshMetrics();
      } else {
    ...
    function startToBody() {
      if (animating || sceneState !== 'hero') return;
      lockedScrollY = window.scrollY || 0;
      sceneState = 'transition';
      animateScene('to-body');
    }

    function startToHero() {
      if (animating || sceneState !== 'body') return;
      lockedScrollY = Math.min(window.scrollY || 0, metrics.bodyStart);
      sceneState = 'transition';
      animateScene('to-hero');
    }
    ...
    function animateScene(direction) {
      ...
      const heroStartY = direction === 'to-body' ? 0 : -metrics.heroLift;
      const heroEndY = direction === 'to-body' ? -metrics.heroLift : 0;
      const heroStartScale = direction === 'to-body' ? 1 : config.heroScale;
      const heroEndScale = direction === 'to-body' ? config.heroScale : 1;
      const heroStartOpacity = direction === 'to-body' ? 1 : 0;
      const heroEndOpacity = direction === 'to-body' ? 0 : 1;
      const bodyStartY = direction === 'to-body' ? metrics.bodyShift : 0;
      const bodyEndY = direction === 'to-body' ? 0 : metrics.bodyShift;
      const bodyStartOpacity = direction === 'to-body' ? 0 : 1;
      const bodyEndOpacity = direction === 'to-body' ? 1 : 0;

      const step = (now) => {
        const elapsed = now - startTime;
        const progress = Math.min(1, elapsed / config.duration);
        const eased = easeInOutCubic(progress);
        hero.style.transform = `translate3d(0, ${lerp(heroStartY, heroEndY, eased)}px, 0) scale(${lerp(
          heroStartScale,
          heroEndScale,
          eased
        ).toFixed(4)})`;
        hero.style.opacity = lerp(heroStartOpacity, heroEndOpacity, eased).toFixed(3);
        bodySection.style.transform = `translate3d(0, ${lerp(bodyStartY, bodyEndY, eased)}px, 0)`;
        bodySection.style.opacity = lerp(bodyStartOpacity, bodyEndOpacity, eased).toFixed(3);
        if (overlay) {
          const overlayValue =
            direction === 'to-body' ? overlayCurve(eased) : overlayCurve(1 - eased);
          overlay.style.opacity = (config.overlayMax * overlayValue).toFixed(3);
        }
    ...
    function finalize(direction) {
      ...
      touchStartY = null;
      if (direction === 'to-body') {
        sceneState = 'body';
        document.body.classList.remove('scene-hero');
        document.body.classList.add('scene-body');
        applyBodyState();
        refreshMetrics();
        window.scrollTo(0, metrics.bodyStart);
      } else {
        sceneState = 'hero';
        document.body.classList.remove('scene-body');
        document.body.classList.add('scene-hero');
        applyHeroState();
        refreshMetrics();
        window.scrollTo(0, 0);
      }
    ...
    function applyHeroState() {
      ...
      bodySection.style.transform = `translate3d(0, ${metrics.bodyShift}px, 0)`;
      bodySection.style.opacity = '0';
      if (overlay) overlay.style.opacity = '0';
    }

    function applyBodyState() {
      hero.style.transform = `translate3d(0, ${-metrics.heroLift}px, 0) scale(${config.heroScale})`;
      hero.style.opacity = '0';
      bodySection.style.transform = 'translate3d(0, 0, 0)';
      bodySection.style.opacity = '1';
      if (overlay) overlay.style.opacity = '0';
    }

    function refreshMetrics() {
      metrics.headerHeight = header ? Math.round(header.getBoundingClientRect().height) : 0;
      if (!metrics.headerHeight) {
        const fallback = parseFloat(
          getComputedStyle(root).getPropertyValue('--headerH').replace('px', '')
        );
        metrics.headerHeight = Number.isFinite(fallback) ? fallback : 0;
      }
      root.style.setProperty('--hero-menu-height', `${metrics.headerHeight}px`);
      const heroHeight = hero.getBoundingClientRect().height || hero.offsetHeight || window.innerHeight;
      metrics.heroLift = Math.max(config.minHeroLift, heroHeight * config.heroLiftRatio);
      metrics.bodyShift = Math.max(config.minBodyShift, heroHeight * config.bodyShiftRatio);
      root.style.setProperty('--cinematic-hero-lift', `${metrics.heroLift}px`);
      root.style.setProperty('--cinematic-body-shift', `${metrics.bodyShift}px`);
      root.style.setProperty('--cinematic-hero-scale', config.heroScale);
      metrics.bodyStart = Math.max(0, bodySection.offsetTop - metrics.headerHeight);
    }
    ...
    function teardownCinematic() {
      ...
      hero.classList.remove('cinematic-section', 'cinematic-hero');
      bodySection.classList.remove('cinematic-section', 'cinematic-body');
      document.body.classList.remove('scene-transition', 'scene-hero', 'scene-body', 'cinematic-ready');
      ...
      bodySection.style.removeProperty('transform');
      bodySection.style.removeProperty('opacity');
    }
  }
