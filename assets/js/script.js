...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /*
   * Mobile parallax fallback for background sections.
   *
   * Mobile Safari and some Android browsers ignore `background-attachment: fixed`,
   * which removes the parallax illusion used on our themed sections.  For
   * handheld breakpoints we dynamically adjust each section’s background
   * position as the user scrolls so the imagery appears anchored to the
   * viewport, matching the desktop experience without altering it.
   */
  const parallaxSectionSelector = [
    '.section.bg-lines',
    '.section.bg-circuit',
    '.section.bg-city',
    '.section.bg-mesh',
  ...
  const parallaxSections = Array.from(document.querySelectorAll(parallaxSectionSelector));
  ...
  const normaliseAxisValue = (value, axis) => {
    if (!value) return '50%';
    const first = value.split(',')[0].trim();
    const mappings = axis === 'x'
      ? { left: '0%', center: '50%', right: '100%' }
      : { top: '0%', center: '50%', bottom: '100%' };
    return mappings[first] || first || '50%';
  };

  const storeSectionBaseline = (section) => {
    const computed = window.getComputedStyle(section);
    section.dataset.parallaxBaseX = normaliseAxisValue(computed.backgroundPositionX, 'x');
    section.dataset.parallaxBaseY = normaliseAxisValue(computed.backgroundPositionY, 'y');
  };

  const applyParallaxPosition = (section) => {
    const baseX = section.dataset.parallaxBaseX || '50%';
    const baseY = section.dataset.parallaxBaseY || '50%';
    const offset = Math.round(window.scrollY - section.offsetTop);
    const operator = offset < 0 ? '-' : '+';
    const magnitude = Math.abs(offset);
    section.style.backgroundPosition = `${baseX} calc(${baseY} ${operator} ${magnitude}px)`;
  };

  const updateParallaxPositions = () => {
    parallaxSections.forEach((section) => {
      applyParallaxPosition(section);
    });
  };

  const requestParallaxFrame = () => {
    if (!parallaxActive || parallaxRafId !== null) return;
  ...
  const enableParallax = () => {
    if (parallaxActive || !parallaxSections.length) return;
    parallaxActive = true;
    parallaxSections.forEach((section) => {
      section.dataset.parallaxOriginalPosition = section.style.backgroundPosition || '';
      storeSectionBaseline(section);
    });
    updateParallaxPositions();
  ...
  const disableParallax = () => {
    ...
    parallaxSections.forEach((section) => {
      const original = section.dataset.parallaxOriginalPosition;
      if (original) {
        section.style.backgroundPosition = original;
      } else {
        section.style.removeProperty('background-position');
      }
      delete section.dataset.parallaxOriginalPosition;
      delete section.dataset.parallaxBaseX;
      delete section.dataset.parallaxBaseY;
    });
  };
  ...
  if (parallaxSections.length) {
    addMediaListener(mobileParallaxQuery, evaluateParallax);
    addMediaListener(reduceMotionQuery, evaluateParallax);
    evaluateParallax();
  }
