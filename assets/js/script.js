...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /*
   * Cinematic scroll controller – orchestrates the premium transition
   * between the hero and the first content section.  The effect only
   * runs when both sections exist on the page (currently the home
   * layout) and is disabled for users who prefer reduced motion.
   */
  class CinematicScrollController {
    constructor({ hero, bodySection, headerEl, indicatorEl, navMenuEl, reducedMotion }) {
      this.hero = hero;
      this.bodySection = bodySection;
      this.headerEl = headerEl;
      this.indicatorEl = indicatorEl;
      this.navMenuEl = navMenuEl;
      ...
      this.menuHeight = 0;
      this.bodyAnchor = 0;
      this.isAtBodyTop = true;
      this.lockedScrollPosition = null;
    }

    init() {
      this.syncOffsets();
      this.bindOffsetListeners();
      if (this.reducedMotion) {
        return;
      }

      this.body.classList.add('cinematic-enabled', 'cinematic-stage-hero');
      this.bodySection.classList.add('cinematic-target');
      this.jumpTo(0);
      this.bindEvents();
    }
    ...
    bindOffsetListeners() {
      ...
      this.onMenuTransition = this.handleMenuTransition.bind(this);
      window.addEventListener('resize', this.onResize);
      window.addEventListener('orientationchange', this.onResize);
      [this.headerEl, this.indicatorEl].forEach((el) => {
        if (!el) return;
        el.addEventListener('transitionend', this.onMenuTransition);
      });
    }
    ...
    handleWheel(event) {
      if (this.reducedMotion || this.isTransitioning || this.isNavigationOpen()) return;
    ...
    handleKeydown(event) {
      ...
      const downKeys = new Set(['pagedown', 'arrowdown', 'space', ' ', 'spacebar']);
      const upKeys = new Set(['pageup', 'arrowup', 'home']);
      const key = event.key.toLowerCase();

      if (this.state === 'hero' && downKeys.has(key)) {
        event.preventDefault();
        this.transitionToBody();
      } else if (this.state === 'body' && this.isAtBodyTop && upKeys.has(key)) {
        event.preventDefault();
        this.transitionToHero();
      }
    }
    ...
    handleTouchStart(event) {
      if (this.reducedMotion || this.isTransitioning || this.isNavigationOpen()) {
        this.touchStartY = null;
        return;
      }
    ...
    handleTouchMove(event) {
      if (this.reducedMotion || this.isTransitioning || this.isNavigationOpen()) return;
      if (typeof this.touchStartY !== 'number') return;
      const currentTouch = event.touches && event.touches[0];
      if (!currentTouch) return;

      const delta = this.touchStartY - currentTouch.clientY;

      if (this.state === 'hero' && delta > 12) {
        event.preventDefault();
        this.transitionToBody();
      } else if (this.state === 'body' && this.isAtBodyTop && delta < -12) {
        event.preventDefault();
        this.transitionToHero();
      }
    }
    ...
    handleResize() {
      ...
      if (this.state === 'hero') {
        this.jumpTo(0);
      } else if (this.state === 'body' && this.isAtBodyTop) {
        this.jumpTo(this.bodyAnchor);
      }
    }

    handleScroll() {
      if (this.isTransitioning && typeof this.lockedScrollPosition === 'number') {
        if (Math.abs(window.scrollY - this.lockedScrollPosition) > 1) {
          this.jumpTo(this.lockedScrollPosition);
        }
        return;
      }
      ...
      if (this.state === 'body') {
        this.isAtBodyTop = window.scrollY <= this.bodyAnchor + 2;
      }
    }
    ...
    isNavigationOpen() {
      return Boolean(this.navMenuEl && this.navMenuEl.classList.contains('open'));
    }
    ...
    getMenuHeight() {
      ...
      const indicatorRect = this.indicatorEl ? this.indicatorEl.getBoundingClientRect() : null;
      if (indicatorRect && indicatorRect.bottom > 0) {
        return Math.round(indicatorRect.height);
      }
      const fallback = getComputedStyle(document.documentElement).getPropertyValue('--headerH');
    ...
    syncOffsets() {
      ...
      document.documentElement.style.setProperty('--hero-menu-offset', `${this.menuHeight}px`);
      this.bodyAnchor = Math.max(0, this.bodySection.offsetTop - this.menuHeight);
    }
    ...
    lockScroll(targetPosition) {
      this.lockedScrollPosition = targetPosition;
      this.body.classList.add('cinematic-locked');
      this.jumpTo(targetPosition);
    }

    unlockScroll() {
      this.body.classList.remove('cinematic-locked');
      this.lockedScrollPosition = null;
    }

    transitionToBody() {
      if (this.state === 'body' || this.isTransitioning) return;
      this.syncOffsets();
      this.lockScroll(0);
      this.isTransitioning = true;
      this.body.classList.add('cinematic-transitioning');

      requestAnimationFrame(() => {
        this.body.classList.remove('cinematic-stage-hero');
        this.body.classList.add('cinematic-stage-body');
      });

      window.setTimeout(() => {
        this.lockedScrollPosition = this.bodyAnchor;
        this.jumpTo(this.bodyAnchor);
        this.finishTransition('body');
      }, this.duration);
    }

    transitionToHero() {
      if (this.state === 'hero' || this.isTransitioning) return;
      this.syncOffsets();
      this.lockScroll(this.bodyAnchor);
      this.isTransitioning = true;
      this.body.classList.add('cinematic-transitioning');

      requestAnimationFrame(() => {
        this.body.classList.remove('cinematic-stage-body');
        this.body.classList.add('cinematic-stage-hero');
      });

      window.setTimeout(() => {
        this.lockedScrollPosition = 0;
        this.jumpTo(0);
        this.finishTransition('hero');
      }, this.duration);
    }

    finishTransition(targetStage) {
      this.state = targetStage;
      this.isTransitioning = false;
      this.body.classList.remove('cinematic-transitioning');
      this.unlockScroll();
      this.touchStartY = null;
  ...
  let primaryBodySection = heroSection ? heroSection.nextElementSibling : null;
  while (
    primaryBodySection &&
    (!primaryBodySection.tagName || primaryBodySection.tagName.toLowerCase() !== 'section')
  ) {
    primaryBodySection = primaryBodySection.nextElementSibling;
  }

  if (heroSection && primaryBodySection && primaryBodySection.tagName && primaryBodySection.tagName.toLowerCase() === 'section') {
    const cinematicScroll = new CinematicScrollController({
      hero: heroSection,
      bodySection: primaryBodySection,
      headerEl: header,
      indicatorEl: headerIndicator,
      navMenuEl: navMenu,
