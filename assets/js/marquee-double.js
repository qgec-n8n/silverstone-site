(function () {
    ...
    function initDoubleMarquee() {
        ...
        document.querySelectorAll('.premium-marquee-container, .logo-slider, .double-marquee-container').forEach(el => el.remove());

        const marquee = createDoubleDeckContainer();
        injectLightbox();

        const gallery = document.getElementById('neural-grid');
        const galleryParent = gallery ? gallery.parentElement : null;

        if (gallery && galleryParent) {
            galleryParent.insertBefore(marquee, gallery.nextSibling);
        } else {
    ...
    function createDoubleDeckContainer() {
        ...
        container.className = 'double-marquee-container';

        const row1 = createMarqueeRow(MARQUEE_IMAGES, 'scroll-right fast');
        const row2 = createMarqueeRow([...MARQUEE_IMAGES].reverse(), 'scroll-left slow');
