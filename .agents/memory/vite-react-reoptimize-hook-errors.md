...
**Misattribution trap ("this NEW page loads wrong / glitches fixed halfway"):** This cascade is **app-wide and cold-start driven**, not route-specific. When a user reports the just-added routes (e.g. `/industry/*`) "load in the wrong sequence with glitches that fix themselves halfway," the cause is almost always this dev artifact — the new routes are simply the ones being cold-loaded/tested right after server restarts, so they hit the duplicate-React window most. Before chasing a per-route bug, prove it isn't this: a **warm** load of the suspect route with a **clean** console (only `[vite] connected` + DevTools notice) rules out a real defect; then confirm the same cascade also fires on Home after a hard restart. The industry/service detail experiences share 100% of the load machinery (boot-script branch in `root.tsx`, `app-experience` state machine, `CoreSpinLoader`, the `ss-service-experience` CSS gate, lazy+Suspense, and external deps) — so a glitch in one but "not" the other is a cold-vs-warm timing coincidence, not a code difference. This is a dev/staging-only artifact; it does not exist in the prerendered production build.
