...
- Claude Fable 5 is an advisor, not a driver. Use it only for bounded questions of direction — positioning, conversion strategy, page structure, substantial webcopy direction, major visual direction — and never for implementation, routine edits, CSS, debugging, validation, or final review. Narrow the decision first on the driving model, ask Fable 5 once in no more than 500 words of context for one preferred direction, then hand the answer back to the driving model to implement. At most one consultation per substantial task.
